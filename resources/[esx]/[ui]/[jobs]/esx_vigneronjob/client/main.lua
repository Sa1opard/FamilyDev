	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('wine_clothes_wine'), value = 'vigne_wear'},
			},
		},
		function(data, menu)
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'kamacho'},
		}
...
AddEventHandler('esx_vigneronjob:hasEnteredMarker', function(zone)
	...
	if zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'wine_traitement'
		CurrentActionMsg  = _U('press_transfo')
		CurrentActionData = {zone= zone}
	end		
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'wine_traitement' then
					TriggerServerEvent('esx_vigneronjob:startTransform', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 150 then
...
RegisterNetEvent('NB:openMenuVigne')
AddEventHandler('NB:openMenuVigne', function()
	OpenMobileVigneActionsMenu()
end)