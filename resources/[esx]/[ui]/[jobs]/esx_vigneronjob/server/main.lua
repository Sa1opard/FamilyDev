	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			...
			if wine == 0 and jus == 0 then
				...
				return
			elseif xPlayer.getInventoryItem('wine').count <= 0 and jus == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_vin_sale'))
				wine = 0
				return
			elseif xPlayer.getInventoryItem('jus_raisin').count <= 0 and wine == 0then
