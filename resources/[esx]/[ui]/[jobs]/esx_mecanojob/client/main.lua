...
function OpenCarPoundMenu()

	ESX.TriggerServerCallback('esx_mecanojob:getCustomers', function(customers)

		local elements = {
			head = {'Client', 'Véhicule', 'Santé', 'Action'},
			rows = {}
		}
		...
		for i=1, #customers, 1 do
			table.insert(elements.rows,
			{
				data = customers[i],
				cols = {
					customers[i].vehicleOwnerName,
					GetLabelText(GetDisplayNameFromVehicleModel(customers[i].vehicle.model)),
					(tonumber(customers[i].vehicle.health)/10) .. '%',
					'{{Sortir|get_out}}'
				}
			})
		end
		...
		ESX.UI.Menu.Open(
			...
			elements,
			function(data, menu)
				if data.value == 'get_out' then
					ESX.Game.SpawnVehicle(data.data.vehicle.model, Config.Zones.CarPoundSpawnPoint.Pos, 316.0, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, data.data.vehicle)
					end)
					TriggerServerEvent('esx_mecanojob:getCarOutOfPound', data.data.id)
				end
				menu.close()
			end,
			function(data, menu)
...
function OpenGetBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_mecanojob:getBlackMoneySociety', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.type == 'item_weapon' then
          menu.close()
          TriggerServerEvent('esx_mecanojob:getItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              if quantity == nil then
                ...
                menu.close()
                TriggerServerEvent('esx_mecanojob:getItem', data.current.type, data.current.value, quantity)
                ESX.SetTimeout(300, function()
                  OpenGetBlackMoney()
                end)
              end
...
function OpenPutBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_mecanojob:getPlayerInventory2', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_mecanojob:putItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              TriggerServerEvent('esx_mecanojob:putItem', data.current.type, data.current.value, tonumber(data2.value))
		...
		function(data, menu)
			if data.current.value == 'vehicle_list' then
				local elements = {
					...
					{label = 'Moto', value = 'Gargoyle'},
					{label = 'Vhc de pret', value = 'Rhapsody'},
					{label = 'Intervention rapide', value ='Wastelander'}
				}
...
AddEventHandler('esx_mecanojob:hasEnteredMarker', function(zone)
	...
	if zone == 'CarPound' then
		CurrentAction     = 'car_pound'
		CurrentActionMsg  = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au menu fourrière.'
		CurrentActionData = {}
	end
    ...
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
                ...
                if CurrentAction == 'car_pound' then
                    OpenCarPoundMenu()
                end
                ...
                if CurrentAction == 'delete_vehicle' then
                    ...
                    local plate = GetVehicleNumberPlateText(vehicle)
                    if hash == GetHashKey('flatbed') or hash == GetHashKey('towtruck2') or hash == GetHashKey('slamvan3') or hash == GetHashKey('Wastelander') or hash == GetHashKey('Gargoyle') or hash == GetHashKey('Rhapsody') then
                        if Config.MaxInService ~= -1 then
        ...
        if IsControlJustReleased(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
	      ...
	      if IsControlJustReleased(0, Keys['F10']) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
