							...
							ESX.Game.SpawnObject('prop_cs_steak', {
								...
								ESX.SetTimeout(data.data.CookingTime, function()
									...
									if data.data.Item == 'tacos' then
										model = 'prop_taco_01'
									elseif data.data.Item == 'hamburger' then
										model = 'prop_cs_hamburger_01'
									end
				...
				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'foodtruck_gears',
					{
						...
						elements = {
							...
							{label = _U('chaise'), 	value = 'prop_table_03_chr'}, --'prop_cs_steak'prop_table_03_chr
...
AddEventHandler('esx_foodtruck:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'foodtruck' then
		...
		if GetEntityModel(entity) == GetHashKey('prop_cs_hamburger_01') then
			CurrentAction     = 'foodtruck_client_hamburger'
			CurrentActionMsg  = _U('take') .. ' ' .. _U('hamburger')
			CurrentActionData = {entity = entity, item = 'hamburger'}
		end
    ...
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'foodtruck' then
                ...
                elseif CurrentAction == 'foodtruck_client_hamburger' or CurrentAction == 'foodtruck_client_tacos' or CurrentAction == 'foodtruck_client_makiriime' then
